...
public class CompilerPipelineTests
{
    ...
    [Fact]
    public void Compile_list_indexing_generates_code()
    {
        var compiler = new CompilerDriver();
        var result = compiler.Compile("let xs = [1, 2, 3]\nprint xs[0]", "test.axom");

        Assert.True(result.Success);
        Assert.Contains("xs[0]", result.GeneratedCode);
    }

    [Fact]
    public void Compile_map_literal_generates_code()
    {
        var compiler = new CompilerDriver();
        var result = compiler.Compile("let m = [\"a\": 1, \"b\": 2]", "test.axom");

        Assert.True(result.Success);
        Assert.Contains("new Dictionary<string, int>", result.GeneratedCode);
    }
    ...
    [Fact]
    public void Compile_channel_send_recv_generates_channel_runtime_code()
    {
        var compiler = new CompilerDriver();
