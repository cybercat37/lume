...
public class RangeBuiltinTypeTests
{
    ...
    [Fact]
    public void Range_with_step_type_checks_for_valid_signature()
    {
        var sourceText = new SourceText("let xs = range(1, 10, 2)\nprint xs", "test.axom");
        var syntaxTree = SyntaxTree.Parse(sourceText);
    ...
    [Fact]
    public void Range_rejects_non_int_step_argument()
    {
        var sourceText = new SourceText("let xs = range(1, 10, \"2\")", "test.axom");
        var syntaxTree = SyntaxTree.Parse(sourceText);
