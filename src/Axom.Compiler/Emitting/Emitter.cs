...
public sealed class Emitter
{
    public string Emit(LoweredProgram program)
    {
        ...
        {
            builder.AppendLine("using Microsoft.Data.Sqlite;");
        }
    ...
    {
        builder.AppendLine("    static SqliteConnection? AxomDbTransactionConnection;");
        builder.AppendLine("    static SqliteTransaction? AxomDbTransaction;");
        builder.AppendLine();
        builder.AppendLine("    static AxomResult<int> AxomDbExec(string sql)");
        builder.AppendLine("    {");
        builder.AppendLine("        return AxomDbExec(sql, new Dictionary<string, string>(StringComparer.Ordinal));");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    static AxomResult<int> AxomDbExec(string sql, Dictionary<string, string> parameters)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (!AxomBindSqlTemplate(sql, parameters, out var boundSql, out var boundParameters, out var bindError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(bindError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (!AxomGetDbContext(out var connection, out var transaction, out var shouldDisposeConnection, out var connectionError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(connectionError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        builder.AppendLine("            using var command = connection.CreateCommand();");
        builder.AppendLine("            command.Transaction = transaction;");
        builder.AppendLine("            command.CommandText = boundSql;");
        builder.AppendLine("            AxomAddDbParameters(command, boundParameters);");
        builder.AppendLine("            var affected = command.ExecuteNonQuery();");
        builder.AppendLine("            return AxomResult<int>.Ok(affected);");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(ex.Message);");
        builder.AppendLine("        }");
        builder.AppendLine("        finally");
        builder.AppendLine("        {");
        builder.AppendLine("            if (shouldDisposeConnection)");
        builder.AppendLine("            {");
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        return AxomDbQuery(sql, new Dictionary<string, string>(StringComparer.Ordinal));");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.AppendLine("        if (!AxomBindSqlTemplate(sql, parameters, out var boundSql, out var boundParameters, out var bindError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<List<Dictionary<string, string>>>.Error(bindError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (!AxomGetDbContext(out var connection, out var transaction, out var shouldDisposeConnection, out var connectionError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<List<Dictionary<string, string>>>.Error(connectionError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("            command.CommandText = boundSql;");
        builder.AppendLine("            AxomAddDbParameters(command, boundParameters);");
        builder.AppendLine("            using var reader = command.ExecuteReader();");
        builder.AppendLine("            var rows = new List<Dictionary<string, string>>();");
        builder.AppendLine("            while (reader.Read())");
        builder.AppendLine("            {");
        builder.AppendLine("                var row = new Dictionary<string, string>(reader.FieldCount, StringComparer.Ordinal);");
        builder.AppendLine("                for (var i = 0; i < reader.FieldCount; i++)");
        builder.AppendLine("                {");
        builder.AppendLine("                    var value = reader.IsDBNull(i) ? string.Empty : Convert.ToString(reader.GetValue(i), System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty;");
        builder.AppendLine("                    row[reader.GetName(i)] = value;");
        builder.AppendLine("                }");
        builder.AppendLine("                rows.Add(row);");
        builder.AppendLine("            }");
        builder.AppendLine("            return AxomResult<List<Dictionary<string, string>>>.Ok(rows);");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<List<Dictionary<string, string>>>.Error(ex.Message);");
        builder.AppendLine("        }");
        builder.AppendLine("        finally");
        builder.AppendLine("        {");
        builder.AppendLine("            if (shouldDisposeConnection)");
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        return AxomDbScalar(sql, new Dictionary<string, string>(StringComparer.Ordinal));");
        builder.AppendLine("    }");
        builder.AppendLine();
        ...
        builder.AppendLine("        if (!AxomBindSqlTemplate(sql, parameters, out var boundSql, out var boundParameters, out var bindError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<string>.Error(bindError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (!AxomGetDbContext(out var connection, out var transaction, out var shouldDisposeConnection, out var connectionError))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<string>.Error(connectionError);");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("            command.CommandText = boundSql;");
        builder.AppendLine("            AxomAddDbParameters(command, boundParameters);");
        builder.AppendLine("            var value = command.ExecuteScalar();");
        builder.AppendLine("            return AxomResult<string>.Ok(value is null || value is DBNull ? string.Empty : Convert.ToString(value, System.Globalization.CultureInfo.InvariantCulture) ?? string.Empty);");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<string>.Error(ex.Message);");
        builder.AppendLine("        }");
        builder.AppendLine("        finally");
        builder.AppendLine("        {");
        builder.AppendLine("            if (shouldDisposeConnection)");
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        if (AxomDbTransactionConnection is not null || AxomDbTransaction is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(\"transaction is already active\");");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var provider = Environment.GetEnvironmentVariable(\"AXOM_DB_PROVIDER\");");
        builder.AppendLine("        var connectionString = Environment.GetEnvironmentVariable(\"AXOM_DB_CONNECTION_STRING\");");
        builder.AppendLine("        if (!string.Equals(provider, \"sqlite\", StringComparison.OrdinalIgnoreCase) || string.IsNullOrWhiteSpace(connectionString))");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(\"configure AXOM_DB_PROVIDER=sqlite and AXOM_DB_CONNECTION_STRING before using db transactions\");");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        builder.AppendLine("            var connection = new SqliteConnection(connectionString);");
        builder.AppendLine("            connection.Open();");
        builder.AppendLine("            var tx = connection.BeginTransaction();");
        builder.AppendLine("            AxomDbTransactionConnection = connection;");
        builder.AppendLine("            AxomDbTransaction = tx;");
        builder.AppendLine("            return AxomResult<int>.Ok(1);");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        if (AxomDbTransaction is null || AxomDbTransactionConnection is null)");
        builder.AppendLine("        {");
        builder.AppendLine("            return AxomResult<int>.Error(\"transaction is not active\");");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        builder.AppendLine("            AxomDbTransaction.Commit();");
        builder.AppendLine("            return AxomResult<int>.Ok(1);");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    static bool AxomGetDbContext(out SqliteConnection connection, out SqliteTransaction? transaction, out bool shouldDisposeConnection, out string error)");
        builder.AppendLine("    {");
        builder.AppendLine("        if (AxomDbTransactionConnection is not null && AxomDbTransaction is not null)");
        builder.AppendLine("        {");
        builder.AppendLine("            connection = AxomDbTransactionConnection;");
        builder.AppendLine("            transaction = AxomDbTransaction;");
        builder.AppendLine("            shouldDisposeConnection = false;");
        ...
        builder.AppendLine("        if (!string.Equals(provider, \"sqlite\", StringComparison.OrdinalIgnoreCase) || string.IsNullOrWhiteSpace(connectionString))");
        builder.AppendLine("        {");
        builder.AppendLine("            connection = null!;");
        builder.AppendLine("            transaction = null;");
        builder.AppendLine("            shouldDisposeConnection = false;");
        builder.AppendLine("            error = \"configure AXOM_DB_PROVIDER=sqlite and AXOM_DB_CONNECTION_STRING before using db builtins\";");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        try");
        builder.AppendLine("        {");
        builder.AppendLine("            connection = new SqliteConnection(connectionString);");
        builder.AppendLine("            connection.Open();");
        builder.AppendLine("            transaction = null;");
        builder.AppendLine("            shouldDisposeConnection = true;");
        builder.AppendLine("            error = string.Empty;");
        builder.AppendLine("            return true;");
        builder.AppendLine("        }");
        builder.AppendLine("        catch (Exception ex)");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    static void AxomAddDbParameters(SqliteCommand command, Dictionary<string, string> parameters)");
        builder.AppendLine("    {");
        builder.AppendLine("        foreach (var pair in parameters)");
        builder.AppendLine("        {");
        builder.AppendLine("            var name = pair.Key;");
        builder.AppendLine("            if (!name.StartsWith(\"@\", StringComparison.Ordinal) && !name.StartsWith(\":\", StringComparison.Ordinal) && !name.StartsWith(\"$\", StringComparison.Ordinal))");
        builder.AppendLine("            {");
        builder.AppendLine("                name = \"@\" + name;");
        builder.AppendLine("            }");
        builder.AppendLine("            command.Parameters.AddWithValue(name, pair.Value);");
        builder.AppendLine("        }");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    static bool AxomBindSqlTemplate(string sql, Dictionary<string, string> parameters, out string boundSql, out Dictionary<string, string> boundParameters, out string error)");
        builder.AppendLine("    {");
        builder.AppendLine("        boundSql = sql;");
        builder.AppendLine("        boundParameters = new Dictionary<string, string>(StringComparer.Ordinal);");
        builder.AppendLine("        error = string.Empty;");
        builder.AppendLine();
        builder.AppendLine("        if (string.IsNullOrWhiteSpace(sql))");
        builder.AppendLine("        {");
        builder.AppendLine("            return true;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var output = new System.Text.StringBuilder(sql.Length);");
        builder.AppendLine("        var inSingleQuotedString = false;");
        builder.AppendLine("        var placeholderCount = 0;");
        builder.AppendLine();
        builder.AppendLine("        for (var i = 0; i < sql.Length; i++)");
        builder.AppendLine("        {");
        builder.AppendLine("            var current = sql[i];");
        builder.AppendLine("            if (inSingleQuotedString)");
        builder.AppendLine("            {");
        builder.AppendLine("                output.Append(current);");
        builder.AppendLine("                if (current == (char)39 && i + 1 < sql.Length && sql[i + 1] == (char)39)");
        builder.AppendLine("                {");
        builder.AppendLine("                    output.Append(sql[++i]);");
        builder.AppendLine("                    continue;");
        builder.AppendLine("                }");
        builder.AppendLine("                if (current == (char)39)");
        builder.AppendLine("                {");
        builder.AppendLine("                    inSingleQuotedString = false;");
        builder.AppendLine("                }");
        builder.AppendLine("                continue;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (current == (char)39)");
        builder.AppendLine("            {");
        builder.AppendLine("                inSingleQuotedString = true;");
        builder.AppendLine("                output.Append(current);");
        builder.AppendLine("                continue;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (current != '{')");
        builder.AppendLine("            {");
        builder.AppendLine("                output.Append(current);");
        builder.AppendLine("                continue;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            var end = sql.IndexOf('}', i + 1);");
        builder.AppendLine("            if (end <= i + 1)");
        builder.AppendLine("            {");
        builder.AppendLine("                error = \"Unterminated SQL placeholder. Expected '}' after '{'.\";");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            var name = sql.Substring(i + 1, end - i - 1).Trim();");
        builder.AppendLine("            if (name.Length == 0)");
        builder.AppendLine("            {");
        builder.AppendLine("                error = \"Empty SQL placeholder '{}'.\";");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (char.IsUpper(name[0]))");
        builder.AppendLine("            {");
        builder.AppendLine("                error = $\"record placeholder '{{{name}}}' is not supported in codegen run mode yet\";");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (!parameters.TryGetValue(name, out var value)");
        builder.AppendLine("                && !parameters.TryGetValue(\"@\" + name, out value)");
        builder.AppendLine("                && !parameters.TryGetValue(\":\" + name, out value)");
        builder.AppendLine("                && !parameters.TryGetValue(\"$\" + name, out value))");
        builder.AppendLine("            {");
        builder.AppendLine("                error = $\"missing SQL parameter '{name}'\";");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            output.Append('@').Append(name);");
        builder.AppendLine("            boundParameters[name] = value;");
        builder.AppendLine("            placeholderCount++;");
        builder.AppendLine("            i = end;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        boundSql = output.ToString();");
        builder.AppendLine("        if (placeholderCount == 0)");
        builder.AppendLine("        {");
        builder.AppendLine("            foreach (var pair in parameters)");
        builder.AppendLine("            {");
        builder.AppendLine("                var key = pair.Key.TrimStart('@', ':', '$');");
        builder.AppendLine("                boundParameters[key] = pair.Value;");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine("        return true;");
        builder.AppendLine("    }");
    ...
    private static void WriteDotNetInteropRuntime(StringBuilder builder)
    {
        ...
        builder.AppendLine("        {");
        builder.AppendLine($"            return AxomResult<T>.Error($\"dotnet type '{{typeName}}' is not allowed. Allowed types: {string.Join(", ", allowedTypes.Select(EscapeString))}.\");");
        builder.AppendLine("        }");
        builder.AppendLine($"        if (({BuildAllowedMethodCondition("typeName", "methodName", whitelist)}) == false)");
        builder.AppendLine("        {");
        builder.AppendLine("            var allowedMethods = typeName switch");
        builder.AppendLine("            {");
        foreach (var (typeName, methods) in whitelist)
        {
            var methodsList = string.Join(", ", methods);
            builder.AppendLine($"                \"{EscapeString(typeName)}\" => \"{EscapeString(methodsList)}\",");
        }
        builder.AppendLine("                _ => string.Empty");
        builder.AppendLine("            };");
        builder.AppendLine("            return AxomResult<T>.Error($\"dotnet method '{typeName}.{methodName}' is not allowed. Allowed methods: {allowedMethods}.\");");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine("        };");
        builder.AppendLine("        var methods = targetType.GetMethods(BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance)");
        builder.AppendLine("            .Where(m => string.Equals(m.Name, methodName, StringComparison.Ordinal))");
        builder.AppendLine("            .ToList();");
        builder.AppendLine("        foreach (var method in methods)");
        builder.AppendLine("        {");
        builder.AppendLine("            var parameters = method.GetParameters();");
        builder.AppendLine("            var isStatic = method.IsStatic;");
        builder.AppendLine("            var expectedCount = isStatic ? args.Length : Math.Max(0, args.Length - 1);");
        builder.AppendLine("            if (parameters.Length != expectedCount)");
        builder.AppendLine("            {");
        builder.AppendLine("                continue;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            object? instance = null;");
        builder.AppendLine("            var argumentOffset = 0;");
        builder.AppendLine("            if (!isStatic)");
        builder.AppendLine("            {");
        builder.AppendLine("                if (args.Length == 0 || !TryConvertArg(args[0], targetType, out instance))");
        builder.AppendLine("                {");
        builder.AppendLine("                    continue;");
        builder.AppendLine("                }");
        builder.AppendLine("                argumentOffset = 1;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            if (!TryConvertArgs(args, argumentOffset, parameters, out var converted))");
        builder.AppendLine("            {");
        builder.AppendLine("                continue;");
        builder.AppendLine("            }");
        builder.AppendLine();
        builder.AppendLine("            try");
        builder.AppendLine("            {");
        builder.AppendLine("                var raw = method.Invoke(instance, converted);");
        ...
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    private static bool TryConvertArgs(object?[] args, int offset, ParameterInfo[] parameters, out object?[] converted)");
        builder.AppendLine("    {");
        builder.AppendLine("        converted = new object?[parameters.Length];");
        builder.AppendLine("        for (var i = 0; i < parameters.Length; i++)");
        builder.AppendLine("        {");
        builder.AppendLine("            if (!TryConvertArg(args[i + offset], parameters[i].ParameterType, out var value))");
        builder.AppendLine("            {");
        builder.AppendLine("                return false;");
        builder.AppendLine("            }");
        ...
        builder.AppendLine("        if (targetType == typeof(object)) { converted = value; return true; }");
        builder.AppendLine("        if (targetType == typeof(int))");
        builder.AppendLine("        {");
        builder.AppendLine("            if (value is int i32) { converted = i32; return true; }");
        builder.AppendLine("            if (value is long l32 && l32 >= int.MinValue && l32 <= int.MaxValue) { converted = (int)l32; return true; }");
        builder.AppendLine("            return false;");
        builder.AppendLine("        }");
