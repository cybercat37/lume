...
| Collections + iterators | List/Map + combinators | Partial | List/Map literals plus `map`/`filter`/`fold`/`each`/`range`/`take`/`skip`/`take_while`/`skip_while`/`zip`/`zip_with` builtins implemented; richer iterator APIs remain planned |
