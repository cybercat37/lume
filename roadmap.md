...
- M18 (Partial): SQL literal surface `sql"""..."""` and method sugar (`.one/.all/.exec`) are implemented, `{param}` runtime binding is implemented, `{Record}` projection works with explicit mapping config, and `axom db verify` (alias `axom db check`) validates against an ephemeral DB with local migration apply.
