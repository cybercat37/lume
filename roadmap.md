...
- Iterator combinators (`map`/`filter`/`fold`/`each`/`count`/`sum`/`any`/`all`) are implemented.
...
- `http { ... }` construction sugar is partially implemented: `baseUrl` (required), `headers`, `timeout`/`timeoutMs`, and `retry` desugar to the HTTP pipeline. Planned follow-up keeps richer defaults (`headers = {}`, `timeout = 30s`) and typed `Retry` record defaults (`retry.max = 0`).
...
- Overload resolution for HTTP builtins with the same name (for example `header`) is type-directed from the left side of `|>`; ambiguity is a compile-time error.
...
- M18 (Partial): SQL literal surface `sql"""..."""` and method sugar (`.one/.all/.exec`) are implemented, `{param}` runtime binding is implemented, `{Record}` projection works with explicit mapping config, `transaction { ... }` sugar is implemented with automatic rollback on early-return and runtime error paths (nested transaction blocks currently rejected), and `axom db verify` (alias `axom db check`) validates against an ephemeral DB with local migration apply.
