...
| Collections + iterators | List/Map + combinators | Partial | List/Map literals plus `map`/`filter`/`fold`/`each`/`range`/`take`/`skip`/`zip`/`zip_with` builtins implemented; richer iterator APIs remain planned |
